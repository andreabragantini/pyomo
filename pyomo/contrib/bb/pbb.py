...
if mpi_available:
    ...
    class ParallelBBSolver_mpi_synchronize(BBSolver):
        ...
        def _solve(self):
            ...
            while len(queue) > 0:
                ...
                while (i < nworkers) and len(queue) > 0:
                    subproblem = queue.pop()
                    handles[i] = self.executor.submit(async_compute_results, subproblem, incumbent_value, sense)
                    i += 1
                #
                # Get results from workers
                #
    ...
    # Synchronous execution of subproblems with Pyro4
    #
    class ParallelBBSolver_mpi_async(BBSolver):
        ...
        def _solve(self):
            ...
            while len(queue) + len(handles) > 0:
                ...
                # decide to generate an incumbent solution.
                #
                while len(free) > 0 and len(queue) > 0:
                    subproblem = queue.pop()
                    i = free.pop()
                    handles[i] = self.executor.submit(async_compute_results, subproblem, incumbent_value, sense)
                #
    ...
    def async_compute_results(subproblem, incumbent_value, sense):
        #print("ASYNC %d" % MPI.COMM_WORLD.Get_rank())
        #print("---")
        #print(type(subproblem))
        #subproblem.debug()
        bound = subproblem.compute_bound()
        if sense*bound <= sense*incumbent_value:
            value, solution = subproblem.get_solution()
